...
def fetch_horses_then_save_json():
    ...
    with open(data_folder + '/horses.json', 'w') as f:
        json.dump(response.json(), f, indent=2)

with DAG(
    dag_id='get_horse_racing_horses',
    start_date=datetime(2024, 1, 1),
    schedule_interval=None,
    catchup=False,
) as dag:
    get_horses = PythonOperator(task_id="get_horses", python_callable=fetch_horses_then_save_json)
    #
    get_horses
