...
def fetch_jockeys_then_save_json():
    ...
    with open(data_folder + '/jockeys.json', 'w') as f:
        json.dump(response.json(), f, indent=2)

with DAG(
    dag_id='get_horse_racing_jockeys',
    start_date=datetime(2024, 1, 1),
    schedule_interval=None,
    catchup=False,
) as dag:
    get_jockeys = PythonOperator(task_id="get_jockeys", python_callable=fetch_jockeys_then_save_json)
    #
    get_jockeys
