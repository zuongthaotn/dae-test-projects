...
def fetch_racecards_then_save_json():
    ...
    with open(data_folder + '/racecards.json', 'w') as f:
        json.dump(response.json(), f, indent=2)

with DAG(
    dag_id='get_horse_racing_racecards',
    start_date=datetime(2024, 1, 1),
    schedule_interval=None,
    catchup=False,
) as dag:
    get_racecards = PythonOperator(task_id="get_racecards", python_callable=fetch_racecards_then_save_json)
    #
    get_racecards
