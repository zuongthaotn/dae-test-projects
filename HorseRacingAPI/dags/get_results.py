...
def fetch_results_then_save_json():
    ...
    with open(data_folder + '/results.json', 'w') as f:
        json.dump(response.json(), f, indent=2)

with DAG(
    dag_id='get_horse_racing_results',
    start_date=datetime(2024, 1, 1),
    schedule_interval=None,
    catchup=False,
) as dag:
    get_results = PythonOperator(task_id="get_results", python_callable=fetch_results_then_save_json)
    #
    get_results
